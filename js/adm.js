...
// Adicionei uma verificação de segurança (se a tabela existir na página atual)
if (tabelaElemento) {
    const tipoInicial = tabelaElemento.dataset.tipo;
    if (tipoInicial) {
        renderizarTabela(tipoInicial);
    }
}

/* --------------------- Clica no botão pra escolher a tabela e troca de cor --------------------- */
const botoes = document.querySelectorAll(".navAside");

botoes.forEach(botao => {
    botao.addEventListener("click", (e) => {
        
        // CORREÇÃO: Se for o botão de sair, deixa o link funcionar normalmente
        if (botao.classList.contains("btn-sair")) {
            return; 
        }

        // Bloqueia o redirecionamento apenas para os botões de abas
        e.preventDefault();
...
function deletarPet(id) {
    if (confirm("Deseja realmente excluir este pet?")) {
        fetch(`http://localhost:8080/pets/${id}`, { method: "DELETE" })
